...
MODEL_URLS = {
    'google/gemma-4-31B-it':               'http://localhost:8000',
    'Qwen/Qwen3-30B-A3B-Instruct-2507':    'http://localhost:8001',
    'google/gemma-4-E4B-it':               'http://localhost:8002',
    'google/gemma-4-E2B-it':               'http://localhost:8003',
}
MODEL_NAMES = list(MODEL_URLS.keys())
...
def model2_request(profile, info, companies, model_name):
    ...
    out = requests.post(
                f"{MODEL_URLS[model_name]}/v1/completions",
                json=payload,
...
all_years = list(range(2010, 2025)) # TODO
print({m: [name['id'] for name in requests.get(f"{url}/v1/models").json()['data']] for m, url in MODEL_URLS.items()})
llm_solutions_lst_train = []
for j in range(15): # TODO
